...
class Controller_Packages extends Controller_Apibase
{
	...
	protected function buildmenu($file = null)
	{
		...
		try
		{
			list($tree) = \Cache::get('api.version_'.\Session::get('version').'.menu');
		}
		catch (\CacheNotFoundException $e)
		{
			// get the unique packages
			$model = \DB::select('package')
				->distinct()
				->from('docblox')
				->where('version_id', '=', \Session::get('version', 0))
				->order_by('package', 'ASC')
			...
			\Cache::set('api.version_'.\Session::get('version').'.menu', array($tree), \Fuel::$env == \Fuel::DEVELOPMENT ? 60 : 3600);
		}

		// get the menu state cookie so we can restore state
		$state = explode(',', str_replace('#node_', '', \Cookie::get('api_menustate', '')));

		// closure to generate an unordered list
		$menu = function ($nodes, $depth = 1, $close = false) use(&$menu, $file, $state)
		{
			// open the list
			...
			if ($nodes)
			{
				...
				foreach ($nodes as $name => $node)
				{
					...
					{
						// this is a file node
						$output .= str_repeat("\t", $depth).'<li id="api_'.$node['id'].'"><div'.($file==$node['hash']?' class="current"':'').'><a href="'.$node['hash'].'" title="'.$node['file'].'">'.basename($node['file']).'</a></div></li>'."\n";
					}
					else
					{
						// new menu item
						$output .= str_repeat("\t", $depth).'<li id="api_'.$name.'"><div>'.$name."</div>\n";

						// and recurse to generate the unordered list for the children
						$output .= $menu($node, $depth+1);
